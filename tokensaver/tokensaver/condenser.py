...
_FILLER_PHRASES = [
    ...
    r"\bhere\b(?=[.,!?])",
    # Hedges and meta-commentary that don't change the request.
    r"\bas you know\b",
    r"\bas you can see\b",
    r"\bof course\b",
    r"\bobviously\b",
    ...
    r"\bkind of\b",
    r"\bmore or less\b",
    # Common request prefixes — what follows is already the instruction.
    r"\bdo the needful\b",
    r"\bdo the need ?ful\b",
...
_REPLACEMENTS = {
    ...
    # Wordy connectors and modifiers.
    r"\bas well as\b": "and",
    ...
    r"\bduring the time that\b": "while",
    ...
    r"\bon the basis of\b": "from",
    r"\bin the absence of\b": "without",
    r"\bso that\b": "so",
    r"\band then\b": "then",
    r"\bafter that\b": "then",
    # Common typos that won't be caught above (preserve meaning).
    r"\bafte rthat\b": "then",
    r"\bso as to\b": "to",
}
